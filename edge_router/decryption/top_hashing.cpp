...
void top_hashing::generate_top_hash()
{
	...
	if (top_file == NULL) perror ("Error opening top hash file");
	else {
	...
	for (int j=0; j<TOP_HASH_SIZE; j++)
		top_hash_map[bitset<7>(random_arr[j]).to_string<char,char_traits<char>,allocator<char> >()] = bitset<7>(j).to_string<char,char_traits<char>,allocator<char> >();
