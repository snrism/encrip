...
string compute_decrypted_address(int addr[], int prefix_len) {
	...
	case 1: {
		...
		anon_tree_string = enc1.to_string<char, char_traits<char> , allocator<
				char> > ();
		...
		// Randomization Algorithm --> Get Random Bits
		{
			...
			randomization_string = rand_obj.get_original_24_bits(
					(long unsigned) randomization_addr);
		}
	}
		break;
	...
	case 18: {
		...
		anon_tree_string += (enc_l1.to_string<char, char_traits<char> ,
				allocator<char> > ());
	...
	final_random_string += top_hash_string + anon_tree_string
			+ randomization_string;
...
int main() {
	...
	/* Generate Top Hashing Data Structure */
	...
	do {
		...
		case IPQM_PACKET: {
			...
			bitset<16> id(ntohs(ip->id));
			string src_string, dst_string;
			string id_string = id.to_string<char, char_traits<char> ,
					allocator<char> > ();

			src_string = source.to_string<char, char_traits<char> , allocator<
					char> > ();
			src_string += id_string.substr(0, 8);

			dst_string = destination.to_string<char, char_traits<char> ,
					allocator<char> > ();
			dst_string = id_string.substr(8, 8);

			int src_addr[5] = {
					(bitset<8> (src_string.substr(0, 8))).to_ulong(),
					(bitset<8> (src_string.substr(8, 8))).to_ulong(),
					(bitset<8> (src_string.substr(16, 8))).to_ulong(), (bitset<
							8> (src_string.substr(24, 8))).to_ulong(), (bitset<
							8> (src_string.substr(32, 8))).to_ulong() };
			int src_prefix_length = 24;
			string src_final_random_string = compute_decrypted_address(
					src_addr, src_prefix_length);

			cout << "Recvd Src: "
					<< (bitset<8> (src_string.substr(0, 8))).to_ulong() << "."
					<< (bitset<8> (src_string.substr(8, 8))).to_ulong() << "."
					<< (bitset<8> (src_string.substr(16, 8))).to_ulong() << "."
					<< (bitset<8> (src_string.substr(24, 8))).to_ulong() << "."
					<< (bitset<8> (src_string.substr(32, 8))).to_ulong()
					<< endl;

			int dst_addr[5] = {
					(bitset<8> (dst_string.substr(0, 8))).to_ulong(),
					(bitset<8> (dst_string.substr(8, 8))).to_ulong(),
					(bitset<8> (dst_string.substr(16, 8))).to_ulong(), (bitset<
							8> (dst_string.substr(24, 8))).to_ulong(), (bitset<
							8> (dst_string.substr(32, 8))).to_ulong() };
			int dst_prefix_length = 24;
			string dst_final_random_string = compute_decrypted_address(
					dst_addr, dst_prefix_length);

			cout << "Recvd Dst: "
					<< (bitset<8> (dst_string.substr(0, 8))).to_ulong() << "."
					<< (bitset<8> (dst_string.substr(8, 8))).to_ulong() << "."
					<< (bitset<8> (dst_string.substr(16, 8))).to_ulong() << "."
					<< (bitset<8> (dst_string.substr(24, 8))).to_ulong() << "."
					<< (bitset<8> (dst_string.substr(32, 8))).to_ulong()
					<< endl;
			...
			ip_header->saddr = htonl(
					(bitset<32> (src_final_random_string)).to_ulong());
			ip_header->daddr = htonl(
					(bitset<32> (dst_final_random_string)).to_ulong());
			...
			cout << "Sending Src: " << (bitset<8> (
					src_final_random_string.substr(0, 8))).to_ulong() << "."
					<< (bitset<8> (src_final_random_string.substr(8, 8))).to_ulong()
					<< "."
					<< (bitset<8> (src_final_random_string.substr(16, 8))).to_ulong()
					<< "."
					<< (bitset<8> (src_final_random_string.substr(24, 8))).to_ulong()
					<< endl;
			cout << "Sending Dst: " << (bitset<8> (
					dst_final_random_string.substr(0, 8))).to_ulong() << "."
					<< (bitset<8> (dst_final_random_string.substr(8, 8))).to_ulong()
					<< "."
					<< (bitset<8> (dst_final_random_string.substr(16, 8))).to_ulong()
					<< "."
					<< (bitset<8> (dst_final_random_string.substr(24, 8))).to_ulong()
					<< endl;
			/* Create Data */
