...
int main(int argc, char **argv)
{
    ...
    do{
        ...
        switch (ipq_message_type(buf)) {
		 ...
		 new_packet = (unsigned char*)malloc(new_packet_len);
//		 memcpy(new_packet,(unsigned char*)m,new_packet_len);	
	...
	memcpy((new_packet + ip_header->ihl*4 + tcp_header->doff*4), data, DATA_SIZE);
	...
	CreatePseudoHeaderAndComputeTcpChecksum(tcp_header, ip_header, data);
        ...
        status = ipq_set_verdict(h, m->packet_id,NF_ACCEPT,BUFSIZE+DATA_SIZE,new_packet);
